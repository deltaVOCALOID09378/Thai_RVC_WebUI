...
if "privateuseone" not in device:
    ...
    try:
        ...
        if torch_directml.device_count() > 0:
            ...
            printt("[คำเตือน] ไม่พบอุปกรณ์ DirectML (No DirectML device found). ระบบจะสลับไปใช้ CPU (Falling back to CPU).")
            device = "cpu"
    except Exception as e:
        printt(f"[คำเตือน] ไม่สามารถเรียกใช้ DirectML ได้ (DirectML init failed): {e}. ระบบจะสลับไปใช้ CPU (Falling back to CPU).")
        device = "cpu"
...
if len(todo) == 0:
    printt("[ข้อมูล] ไม่มีไฟล์เสียงให้ประมวลผลในรอบนี้ (No features to extract in this batch).")
else:
    printt(f"[ข้อมูล] จำนวนไฟล์ที่ต้องประมวลผลทั้งหมด (Total files to process): {len(todo)}")
        ...
        try:
            if file.endswith(".wav"):
                ...
                if idx % n == 0 or idx == len(todo) - 1:
                    printt(f"📊 สถานะ (Status): [{idx+1}/{len(todo)}] | ไฟล์ (File): {file} | ขนาด (Shape): {feats.shape}")
        except:
            printt(f"[ข้อผิดพลาด] เกิดปัญหาขณะประมวลผล (Error processing) {file}:\n{traceback.format_exc()}")
