...
def export_onnx(ModelPath, ExportedPath):
    try:
        print(
            f"[ONNX Export] กำลังโหลดโมเดลต้นฉบับ (Loading original model): {ModelPath}"
        )
        ...
        test_phone_lengths = torch.tensor(
            [200]
        ).long()  # ความยาวของ Hidden unit (Length of hidden unit)
        test_pitch = torch.randint(
            size=(1, 200), low=5, high=255
        )  # ความถี่พื้นฐาน F0 (Pitch in Hz)
        ...
        if not check:
            raise RuntimeError(
                "การลดรูปโมเดล ONNX ล้มเหลว (ONNX simplification failed)"
            )
        ...
        error_msg = (
            f"❌ เกิดข้อผิดพลาดในการส่งออก (Export Error):\n{traceback.format_exc()}"
        )
        print(error_msg)
        return error_msg
