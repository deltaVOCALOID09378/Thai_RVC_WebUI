...
class Slicer:
    def __init__(
        ...
        if not min_length >= min_interval >= hop_size:
            raise ValueError(
                "เงื่อนไขที่ต้องปฏิบัติตาม: min_length >= min_interval >= hop_size"
            )
...
def main():
    parser = ArgumentParser(
        description="เครื่องมือตัดแบ่งไฟล์เสียงพร้อมระบบลดเสียงรบกวน"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=44100,
        help="บังคับ Sample Rate (เช่น 40000, 44100, 48000)",
    )
    parser.add_argument(
        "--db_thresh",
        type=float,
        default=-40,
        help="ค่า Threshold สำหรับตรวจจับความเงียบ (dB)",
    )
