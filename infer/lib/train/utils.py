...
def load_checkpoint(
    ...
    for k, v in state_dict.items():
        if k in saved_state_dict:
            ...
            logger.info(
                f"ไม่พบคีย์ '{k}' ใน Checkpoint (อาจเป็นเลเยอร์ใหม่) จะใช้ค่าเริ่มต้น"
            )
            new_state_dict[k] = v
    ...
    if optimizer is not None and load_opt == 1 and "optimizer" in checkpoint_dict:
        try:
            ...
            logger.warning(
                f"ไม่สามารถโหลดสถานะ Optimizer ได้ อาจมีการเปลี่ยนแปลงโครงสร้าง: {e}"
            )

    logger.info(
        f"ดึงข้อมูล Checkpoint: '{checkpoint_path}' ที่รอบฝึกที่ (Epoch): {iteration} สำเร็จ"
    )
...
def save_checkpoint(
    ...
    """
    logger.info(
        f"กำลังบันทึกสถานะโมเดลและ Optimizer ที่รอบฝึกที่ {iteration} ไปยัง {checkpoint_path}"
    )
...
def latest_checkpoint_path(dir_path: str, regex: str = "G_*.pth") -> Optional[str]:
    ...
    f_list.sort(
        key=lambda f: int("".join(filter(str.isdigit, os.path.basename(f))) or -1)
    )
...
def get_hparams(init: bool = True) -> HParams:
    ...
    parser = argparse.ArgumentParser(
        description="ตั้งค่าพารามิเตอร์สำหรับการฝึกสอน (Training Configuration)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="ความถี่ในการบันทึก Checkpoint (จำนวนรอบ)",
    )
    parser.add_argument(
        "-te",
        "--total_epoch",
        type=int,
        required=True,
        help="จำนวนรอบการฝึกสอนรวมทั้งหมด",
    )
    parser.add_argument(
        "-bs", "--batch_size", type=int, required=True, help="ขนาดของ Batch"
    )
    parser.add_argument(
        "-e",
        "--experiment_dir",
        type=str,
        required=True,
        help="ชื่อโฟลเดอร์สำหรับเก็บผลลัพธ์การทดลอง",
    )
    parser.add_argument(
        "-sr",
        "--sample_rate",
        type=str,
        required=True,
        help="อัตราการสุ่มสัญญาณเสียง (เช่น 32k/40k/48k)",
    )
    parser.add_argument(
        "-v", "--version", type=str, required=True, help="เวอร์ชันของสถาปัตยกรรมโมเดล"
    )
    parser.add_argument(
        "-f0",
        "--if_f0",
        type=int,
        required=True,
        help="เปิดใช้งาน F0 (Pitch) หรือไม่ (1=เปิด, 0=ปิด)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="เส้นทางไฟล์โมเดลตั้งต้นของ Generator (Pretrained)",
    )
    parser.add_argument(
        "-pd",
        "--pretrainD",
        type=str,
        default="",
        help="เส้นทางไฟล์โมเดลตั้งต้นของ Discriminator (Pretrained)",
    )
    parser.add_argument(
        "-g",
        "--gpus",
        type=str,
        default="0",
        help="รหัสการ์ดจอที่จะใช้งาน (คั่นด้วย -)",
    )
    parser.add_argument(
        "-sw",
        "--save_every_weights",
        type=str,
        default="0",
        help="ความถี่ในการแตกไฟล์น้ำหนัก (Weights Extraction)",
    )
    parser.add_argument(
        "-l",
        "--if_latest",
        type=int,
        required=True,
        help="บันทึกเฉพาะไฟล์ Checkpoint ล่าสุดหรือไม่ (1=ใช่, 0=ไม่)",
    )
    parser.add_argument(
        "-c",
        ...
        type=int,
        required=True,
        help="โหลดข้อมูลทั้งหมดเข้าสู่ GPU เลยหรือไม่ (1=ใช่, 0=ไม่)",
    )
...
def get_logger(model_dir: str, filename: str = "train.log") -> logging.Logger:
    ...
    if not logger.handlers:
        file_handler = logging.FileHandler(
            os.path.join(model_dir, filename), encoding="utf-8"
        )
