...
def get_rmvpe(
    ...
    if not os.path.exists(model_path):
        logger.error(f"ไม่พบไฟล์โมเดล RMVPE ที่เส้นทาง: {model_path} กรุณาตรวจสอบความถูกต้อง")
    ...
    try:
        ...
        logger.error("ไม่สามารถนำเข้าโมดูล infer.lib.rmvpe ได้ กรุณาตรวจสอบโครงสร้างโฟลเดอร์ของโปรเจกต์")
        raise e
    except RuntimeError as e:
        logger.error(f"เกิดข้อผิดพลาดในการประมวลผลของ PyTorch ขณะโหลดค่าน้ำหนัก: {str(e)}")
        raise e
